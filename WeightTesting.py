...
from DifferentCNNTesting.Save.saveDataArray import save_dataset
...
parser.add_argument('--folds', help='Number of folds for cross validation', default=n_fold)
parser.add_argument('--models', help='File that contain the model configuration in python', default='DefaultModelConfiguration')
parser.add_argument('--save_best', help='Save the best weight for the model', default=True)
...
parser.add_argument('--save_folder_data', help='Please provide the save data directory', default='npdata/')
...
if not os.path.exists(args.save_folder_data):
    os.mkdir(args.save_folder_data)
generators = []
...
save_dataset(args.save_folder_data, 'test_image_set', test_image_set)
save_dataset(args.save_folder_data, 'test_label_set', test_label_set)
networks_history = {}
for index, network in enumerate(networks_list):
    print(network)
    history = []
    fold = 0
    for train_index, validation_index in KFold(int(args.folds)).split(cross_set):
        train_set, train_label, validation_set, validation_label = data_creation.create_train_validation_dataset(save_statistics, args.data_folder, validation_index, train_index, fold, labels, network)
        save_model_path = args.result_folder + '/' + network + '/'
        model_weight_save_callback = call_creation.saving_callbacks(save_model_path, fold)
        # Salvare l'esecuzione sul logfile in formato csv
        log_monitoring = keras.callbacks.CSVLogger(save_statistics + network + '-trainingLOG.csv', separator=';',
                                                             append=True)
        ...
        history.append(model.fit_generator(
            ...
            callbacks=[model_weight_save_callback, log_monitoring]))
        fold = fold + 1
